...
def load_data() -> dict:
    ...
    return {
        "movies": {},
        "users": {},        # { user_id: { "count": int } }
        "next_id": 1,
        "ad_channels": [],  # [ { "username": "@ch", "title": "Nom", "invite": "https://..." } ]
    }
...
def increment_user_count(user_id: int) -> int:
    """Foydalanuvchi kino so'rov sonini +1 qiladi, yangi sonni qaytaradi."""
    data = load_data()
    uid = str(user_id)
    if uid not in data["users"]:
        data["users"][uid] = {"count": 0}
    ...
    for ch in data["ad_channels"]:
        ...
        try:
            member = await context.bot.get_chat_member(
                chat_id=username, user_id=user_id
            )
...
def build_subscribe_keyboard(not_subscribed: list, movie_id: str) -> InlineKeyboardMarkup:
    ...
    buttons.append([
        InlineKeyboardButton(
            ...
            callback_data=f"check_sub_{movie_id}"
        )
    ])
...
async def handle_movie_request(
    update: Update, context: ContextTypes.DEFAULT_TYPE, movie_id: str
):
...
async def ad_channels_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channels:
        lines = [f"  {i+1}. *{ch.get('title', ch['username'])}* — `{ch['username']}`"
        ...
        if not channels:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🔙 Orqaga", callback_data="ad_channels_menu")]]
                ),
            )
            return
        await query.edit_message_text(
            "➖ O'chirmoqchi bo'lgan kanal *raqamini* kiriting:\n\n"
            + "\n".join(f"{i+1}. {ch.get('title','?')} — `{ch['username']}`"
                        for i, ch in enumerate(channels))
            + "\n\nBekor qilish: /cancel",
            parse_mode="Markdown",
        )
...
async def add_movie_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_photo(
            chat_id=OPEN_CHANNEL, photo=photo,
    ...
    await msg.reply_text(
        f"✅ *Kino saqlandi!*\n\n"
        f"🆔 ID: `{movie_id}`\n🎬 Nom: {title}\n"
        f"🔗 `https://t.me/{BOT_USERNAME}?start=movie_{movie_id}`\n\n{ch_status}",
        parse_mode="Markdown",
    )
...
async def add_ad_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2:
        await update.message.reply_text(
            ...
            "`@kinolar_uz | 🎬 Kinolar | https://t.me/+AbCd`\n\n"
    ...
    if username in existing:
        await update.message.reply_text(
            f"⚠️ `{username}` allaqachon ro'yxatda!", parse_mode="Markdown"
        )
    else:
        data["ad_channels"].append({"username": username, "title": title, "invite": invite})
        save_data(data)
        await update.message.reply_text(
            f"✅ *{title}* (`{username}`) qo'shildi!\n"
            f"Jami reklama kanallari: *{len(data['ad_channels'])}* ta",
            parse_mode="Markdown",
        )
...
async def remove_ad_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"❌ Noto'g'ri raqam. 1 dan {len(channels)} gacha kiriting.\nQayta kiriting yoki /cancel"
        )
        return REMOVE_AD_CHANNEL
    ...
    save_data(data)
    await update.message.reply_text(
        f"✅ *{removed.get('title', removed['username'])}* o'chirildi!\n"
        f"Qolgan kanallar: *{len(channels)}* ta",
        parse_mode="Markdown",
    )
    kb = [[InlineKeyboardButton("🔙 Kanallar menyusi", callback_data="ad_channels_menu")]]
    await update.message.reply_text("Menyuga qaytish:", reply_markup=InlineKeyboardMarkup(kb))
...
# ============================================================
# 🌐 Flask & Webhook server (Render uchun)
# ============================================================
...
@app_flask.route("/")
def index():
    return "🎬 Kino Bot v2.0 Webhook ishlayapti!"
...
def main():
    ...
    delete_conv = ConversationHandler(
        ...
        states={
            DELETE_MOVIE_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_movie_id)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    ...
    add_ad_conv = ConversationHandler(
        ...
        states={
            ADD_AD_CHANNEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_ad_channel_handler)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )

    remove_ad_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(button_handler, pattern="^remove_ad_channel$")],
        states={
            REMOVE_AD_CHANNEL: [MessageHandler(filters.TEXT & ~filters.COMMAND, remove_ad_channel_handler)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
    ...
    app.add_handler(add_ad_conv)
    app.add_handler(remove_ad_conv)
    ...
    if RENDER_URL:
        ...
        app.start()
        logger.info(f"🌐 Webhook ulandi: {webhook_url}")
        app_flask.run(host="0.0.0.0", port=PORT)
    else:
        # Lokal test uchun Polling rejimi
        logger.info("🎬 Kino bot v2.0 Polling rejimida ishga tushdi!")
...
if __name__ == "__main__":
    main()
            ...
            # Bot kanal adminı emas — baribir ko'rsatamiz (ehtiyot uchun)
            not_subscribed.append(ch)
...
# ============================================================
# 🎬 Kino so'rovi — majburiy obuna bilan
# ============================================================
async def handle_movie_request(
    update: Update, context: ContextTypes.DEFAULT_TYPE, movie_id: str
):
    ...
    # Har SUBSCRIBE_EVERY ta so'rovda obunani tekshir
    if count % SUBSCRIBE_EVERY == 0:
        not_subscribed = await check_subscriptions(context, user_id)
        if not_subscribed:
            kb = build_subscribe_keyboard(not_subscribed, movie_id)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Obuna tekshirish — admin emas ham ishlatishi mumkin
    if query.data.startswith("check_sub_"):
        await check_sub_callback(update, context)
        return
    ...
    if cb == "admin_panel":
        ...
        await query.edit_message_text(
            ...
            "Quyidagi formatda yuboring:\n"
            "`@username | Kanal nomi | https://t.me/invite_link`\n\n"
            "Misol:\n`@kinolar_uz | 🎬 Kinolar | https://t.me/+AbCdEfGh`\n\n"
            "Invite link bo'lmasa oxiridagi qismni tashlab keting:\n"
            "`@kinolar_uz | 🎬 Kinolar`\n\n"
            "Bekor qilish: /cancel",
            parse_mode="Markdown",
        )
...
# ============================================================
# ➕ Kino qo'shish — ConversationHandler
# ============================================================
async def add_movie_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if msg.forward_from_chat and msg.forward_from_chat.id == PRIVATE_CHANNEL:
...
# ============================================================
# 📡 Reklama kanali qo'shish
# ============================================================
async def add_ad_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Format: @username | Kanal nomi | https://t.me/invite
    yoki:   @username | Kanal nomi
    """
    text  = update.message.text.strip()
    parts = [p.strip() for p in text.split("|")]
...
def main():
    ...
    logger.info("🎬 Kino bot v2.0 ishga tushdi!")
    app.run_polling(allowed_updates=Update.ALL_TYPES)
