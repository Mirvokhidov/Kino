"""
🎬 Telegram Kino Bot — v2.0 (Tozalangan Webhook va 404/405 Fix)
"""
...
# Keraksiz sariq eslatmalarni yashirish
warnings.filterwarnings("ignore", category=PTBUserWarning)

# ============================================================
# ⚙️ SOZLAMALAR
# ============================================================
# DIQQAT: Shu yerga o'zingizning aniq tokeningizni qo'ying!
...
(
    ADD_MOVIE_FILE, ADD_MOVIE_TITLE, ADD_MOVIE_DESC, ADD_MOVIE_POSTER,
    DELETE_MOVIE_ID, BROADCAST_TEXT, ADD_AD_CHANNEL, REMOVE_AD_CHANNEL,
) = range(8)
...
def load_data() -> dict:
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"movies": {}, "users": {}, "next_id": 1, "ad_channels": []}
...
def build_subscribe_keyboard(not_subscribed: list, movie_id: str) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton("✅ Obuna bo'ldim, tekshir", callback_data=f"check_sub_{movie_id}")])
...
async def check_sub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_subscribed:
        ...
        try:
            await context.bot.copy_message(
                chat_id=user_id, from_chat_id=PRIVATE_CHANNEL, message_id=movie["message_id"],
            )
...
async def ad_channels_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ch_text = "\n".join([f"  {i+1}. *{ch.get('title', ch['username'])}* — `{ch['username']}`" for i, ch in enumerate(channels)]) if channels else "  _(hozircha yo'q)_"

    text = f"📡 *Reklama kanallari* ({len(channels)} ta)\n\n{ch_text}\n\nℹ️ Foydalanuvchi har *{SUBSCRIBE_EVERY}* ta kino so'rovda obunaga yo'naltiriladi."
    keyboard = [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cb = query.data
    if cb == "admin_panel": await show_admin_panel(update, context)
    elif cb == "ad_channels_menu": await ad_channels_menu(update, context)
    elif cb == "add_ad_channel":
        await query.edit_message_text("📡 Yangi reklama kanalini qo'shish.\n`@username | Kanal nomi | https://t.me/...`\nBekor qilish: /cancel", parse_mode="Markdown")
        return ADD_AD_CHANNEL
    elif cb == "remove_ad_channel":
        data = load_data()
        if not data["ad_channels"]:
            await query.edit_message_text("📡 O'chirish uchun kanal yo'q.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="ad_channels_menu")]]))
            return
        await query.edit_message_text("➖ O'chirmoqchi bo'lgan kanal *raqamini* kiriting:\nBekor qilish: /cancel", parse_mode="Markdown")
        return REMOVE_AD_CHANNEL
    elif cb == "add_movie":
        await query.edit_message_text("🎬 Kinoni *yopiq kanalga* yuboring, so'ng xabarni botga *forward* qiling.\nBekor qilish: /cancel", parse_mode="Markdown")
        return ADD_MOVIE_FILE
    elif cb == "delete_movie":
        await query.edit_message_text("❌ O'chirmoqchi bo'lgan kino *ID* sini kiriting:\nBekor qilish: /cancel", parse_mode="Markdown")
        return DELETE_MOVIE_ID
    elif cb == "list_movies":
        data = load_data()
        text = "\n".join([f"🎬 ID: `{mid}` — {m['title']}" for mid, m in data["movies"].items()]) if data["movies"] else "📋 Hozircha kinolar yo'q."
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="admin_panel")]]), parse_mode="Markdown")
    elif cb == "stats":
        data = load_data()
        text = f"📊 *Statistika*\n\n👥 Foydalanuvchilar: *{len(data['users'])}*\n🎬 Kinolar soni: *{len(data['movies'])}*\n📡 Reklama kanallari: *{len(data['ad_channels'])}*"
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="admin_panel")]]), parse_mode="Markdown")
    elif cb == "broadcast":
        await query.edit_message_text("📢 Yubormoqchi bo'lgan *reklama matnini* kiriting:\nBekor qilish: /cancel", parse_mode="Markdown")
        return BROADCAST_TEXT
    elif cb == "settings":
        text = f"⚙️ *Sozlamalar*\n\n🤖 Bot: @{BOT_USERNAME}\n📢 Ochiq kanal: {OPEN_CHANNEL}\n🔒 Yopiq kanal ID: `{PRIVATE_CHANNEL}`\n👨‍💼 Admin(lar): {', '.join(str(a) for a in ADMIN_IDS)}\n🔄 Obuna har: *{SUBSCRIBE_EVERY}* so'rovda"
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="admin_panel")]]), parse_mode="Markdown")
...
async def add_movie_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.photo: return ADD_MOVIE_POSTER
    data = load_data()
    ...
    data["movies"][movie_id] = {"title": title, "description": desc, "message_id": context.user_data["movie_msg_id"], "poster": photo}
    save_data(data)
    
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("🎬 Kinoni ko'rish", url=f"https://t.me/{BOT_USERNAME}?start=movie_{movie_id}")]])
    try:
        await context.bot.send_photo(chat_id=OPEN_CHANNEL, photo=photo, caption=f"🎬 *{title}*\n\n📖 {desc}\n\n🎟 Kod: `{movie_id}`", reply_markup=kb, parse_mode="Markdown")
        ch_status = f"✅ Post {OPEN_CHANNEL} kanaliga yuborildi!"
    except Exception as e:
        ch_status = f"⚠️ Kanalga yuborishda xato: {e}"
        
    await update.message.reply_text(f"✅ *Kino saqlandi!*\n🆔 ID: `{movie_id}`\n{ch_status}", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")]]))
...
async def delete_movie_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if movie_id in data["movies"]:
        ...
        save_data(data)
        await update.message.reply_text(f"✅ O'chirildi!", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")]]))
    else:
        await update.message.reply_text(f"❌ ID topilmadi.", parse_mode="Markdown")
    return ConversationHandler.END
...
async def add_ad_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2: return ADD_AD_CHANNEL
    username = parts[0] if parts[0].startswith("@") else "@" + parts[0]
    data = load_data()
    data["ad_channels"].append({"username": username, "title": parts[1], "invite": parts[2] if len(parts) >= 3 else f"https://t.me/{username.lstrip('@')}"})
    save_data(data)
    await update.message.reply_text(f"✅ Qo'shildi!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kanallar menyusi", callback_data="ad_channels_menu")]]))
    return ConversationHandler.END

async def remove_ad_channel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: idx = int(update.message.text.strip()) - 1
    except ValueError: return REMOVE_AD_CHANNEL
    data = load_data()
    if 0 <= idx < len(data["ad_channels"]):
        data["ad_channels"].pop(idx)
        save_data(data)
    await update.message.reply_text("✅ O'chirildi!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kanallar menyusi", callback_data="ad_channels_menu")]]))
    return ConversationHandler.END
...
async def broadcast_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📢 Yuborildi: {sent}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")]]))
    return ConversationHandler.END
...
# ============================================================
# 🚀 Webhookni Qotib Qolmasligi Uchun Himoyalangan Ishga Tushirish
# ============================================================
def main():
    ...
    if RENDER_URL:
        # DIQQAT: Token o'rniga oddiy "/webhook" yo'li ishlatildi
        logger.info(f"🌐 Webhook ishlanmoqda: {RENDER_URL}/webhook")
        try:
            app.run_webhook(
                listen="0.0.0.0",
                port=PORT,
                url_path="webhook", # <--- Eng muhim tuzatish shu yerda!
                webhook_url=f"{RENDER_URL}/webhook", 
                allowed_updates=Update.ALL_TYPES,
                drop_pending_updates=True # <--- Qotib qolgan 9 ta xabarni tozalaydi
            )
        except Exception as e:
